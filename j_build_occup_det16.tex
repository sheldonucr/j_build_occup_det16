\documentclass[acmtodaes]{acmtrans2m}
...
\begin{abstract}
  ...
  on a realistic building simulation program, EnergyPlus, from Department
  of Energy. EnergyPlus can model the various time-series inputs to a
  ...
  Two machine learning based approaches for detecting human
  occupancy of a smart building are applied herein, namely: support
  vector regression (SVR) method and recurrent neural network (RNN)
  method. Experimental results with SVR
  ...
  indicates that SVR is a viable option for occupancy detection.
  In RNN method, Elman's RNN (ELNN) can estimate
...
In this article, we propose a recurrent neural network based method to
detect the occupancy behavior of a building through the temperature
and/or possible heat source information. Two machine learning based
methods: supporting vector machine (SVM) and recurrent neutral
networks (RNN) for smart buildings through the thermal sensor
...
datasets. Ambient factors, room temperature, and/or HVAC power were
selected as features to train Elman's recurrent neural network
(ELNN). In SVR model, two sets of features are offered to feed off the
