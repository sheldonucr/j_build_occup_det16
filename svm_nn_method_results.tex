...
Here working time has a strong correlation with the employee common
schedule, which is a relatively data set to acquire. Therefore,
...
temperatures data as an output. In this paper, we use the outdoor
...
important metric related to number of employees in a specific office
...
non-linear related, where SVR and neural network are a good choices to
...
nonlinear problem used for occupancy detection.
...
In this section, we will illustrate how we measure error variation for
...
In this part of section, we evaluate the SVR detection model using
...
model. The most important two parameters in SVR model is penalty $C$
...
$O_i^{EP}$ denotes the real value of occupancy generated from
EnergyPlus. We calculate the average error and the error ratio by
$\frac{1}{n}\sum\nolimits_n {{e_i}}$ and
...
among variables in statical modelling. The focus is on the
...
forty-five degression line where the targets equal the outputs from
